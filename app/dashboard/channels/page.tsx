...
export default function ChannelsPage() {
  ...
  const load = () => {
    fetch('/api/admin/channels', { headers: headers() })
      .then(r => r.json())
  ...
  const checkHealth = async () => {
    setChecking(true)
    const res = await fetch('/api/admin/health', { method: 'POST', headers: headers() })
    const data = await res.json()
    if (data.channels) {
      setChannels(prev => prev.map(ch => {
        const updated = data.channels.find((c: any) => c.id === ch.id)
        return updated ? { ...ch, healthy: updated.healthy ? 1 : 0, latency_ms: updated.latency_ms } : ch
      }))
    }
    setChecking(false)
  }
