 ...
 * 通过渠道配置计算成本（兼容旧逻辑）
 */
export function calculateCostWithChannel(
  usage: { total_tokens: number },
  rateMultiplier: number,
  baseCostPer1K: number = DEFAULT_COST_PER_1K
): number {
  return parseFloat(((usage.total_tokens / 1000) * rateMultiplier * baseCostPer1K).toFixed(10))
}
